...
- Turn off the risky 440BX features: `CONFIG_NO_HZ_IDLE` **off**, `CONFIG_CPU_FREQ`
  **off** (Katmai has no SpeedStep), `HZ_100`, and boot with
  `clocksource=jiffies tsc=unstable`.
- `CONFIG_ACPI_VIDEO` off (it fights `thinkpad_acpi` for the backlight).
- Keep the in-tree `neofb` NeoMagic framebuffer (the console path).

## Consequences

Stable boots on the 440BX.  The kernel stays on an LTS line with in-tree
NeoMagic support and no reliance on DRM/KMS for the panel.
